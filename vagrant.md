...
```
- multiple vm 환경에서 box로 package화할 때는 ```vagrant package default```와 같은 형식으로 이름을 지정해줘야 한다. (이러면 자동으로 package.box 로 생성됨)

2. package 한 box로 신규 box를 생성한다.
```
vagrant box add mynewbox mynew.box
```
- 경로는 C:\Users\사용자명\.vagrant.d\boxes 에 저장된다.

3. 신규 box를 사용하기 위해서 기존의 박스를 제거하고 init 명령어로 신규 box를 설정한다.
```
vagrant destroy [해당 box명]
rm Vagrantfile
vagrant init mynewbox
```

- multiple vm의 경우엔 다음과 같이 진행한다.
```
vagrant up mynewbox
```

4. 생성한 Box를 가져오려면 다음의 명령어를 입력하면 된다.

```
vagrant box add minion2 minion2.box
```
